...
class BasicAGAgent:
    ...
    The agent always selects the best available action.
    """
    def __init__(self, learn_rate, discount_factor, env) -> None:
        """
        Creates a BasicAGAgent using the given parameters.
        """
    ...
    def reset(self):
        """
        Resets the agent's Q Table. Must be called at least once before
        training or selecting actions.
        """
    ...
    def get_action(self, obs: tuple, avoid_reps=None):
        """
        Returns the selected action for the state `obs`.
        """
    ...
    def update(self, obs, action, reward, next_obs):
        """
        Updates the agent's Q-Table using the reward of the given action
...
class RandomBestOfX:
    """
    Reinforcement Learning agent for the Gamification Preference Environment.
    Implements an `epsilon` probability of randomly picking an action from the X best available.
    """
